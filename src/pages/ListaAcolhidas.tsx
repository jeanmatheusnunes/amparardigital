...
export function ListaAcolhidas() {
  const [acolhidas, setAcolhidas] = useState<Pessoa[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const fetchAcolhidas = async () => {
      try {
        const data = await api.pessoas.listar('acolhida');
        setAcolhidas(data);
      } catch (error) {
        console.error("Erro ao buscar acolhidas:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Tem certeza que deseja excluir este cadastro?')) {
      try {
        ...
        setAcolhidas(acolhidas.filter(a => a.id !== id));
      } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Lista de Acolhidas</h1>
          <p className="text-sm text-gray-500 mt-1">Gerenciamento de mulheres em situação de vulnerabilidade</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="p-8 text-center text-gray-500">Nenhuma acolhida cadastrada.</div>
          ) : (
        ...
        {!isLoading && acolhidas.length > 0 && (
          <div className="p-4 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500">
            <span>Mostrando {acolhidas.length} registros</span>
