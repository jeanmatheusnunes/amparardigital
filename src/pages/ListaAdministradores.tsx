...
export function ListaAdministradores() {
  ...
  const carregarAdministradores = async () => {
    ...
    setError(null);
    try {
      const data = await api.pessoas.listar('administrador');
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Tem certeza que deseja excluir este administrador?')) {
      try {
        ...
        setAdministradores(administradores.filter(p => p.id !== id));
      } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Gerenciamento de acessos e gestores do sistema</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="p-8 text-center text-gray-500">Carregando administradores...</div>
          ) : error ? (
        ...
        <div className="p-4 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500">
          <span>Mostrando {administradores.length} registros</span>
