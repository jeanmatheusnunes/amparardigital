...
export function ListaDoadores() {
  ...
  const carregarDoadores = async () => {
    ...
    setError(null);
    try {
      const data = await api.pessoas.listar('doador');
      setDoadores(data);
    } catch (err) {
      console.error("Erro ao carregar doadores:", err);
      setError("Não foi possível carregar a lista de doadores.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Tem certeza que deseja excluir este doador?')) {
      try {
        ...
        setDoadores(doadores.filter(p => p.id !== id));
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Lista de Doadores</h1>
          <p className="text-sm text-gray-500 mt-1">Gerenciamento de parceiros e contribuintes</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="p-8 text-center text-gray-500">Carregando doadores...</div>
          ) : error ? (
            <div className="p-8 text-center text-red-500">{error}</div>
          ) : doadores.length === 0 ? (
            <div className="p-8 text-center text-gray-500">Nenhum doador encontrado.</div>
          ) : (
                  ...
                  <tr key={pessoa.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button className="p-2 text-gray-400 hover:text-pink-600 hover:bg-pink-50 rounded-lg transition-colors" title="Ver Histórico">
        ...
        <div className="p-4 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500">
          <span>Mostrando {doadores.length} registros</span>
